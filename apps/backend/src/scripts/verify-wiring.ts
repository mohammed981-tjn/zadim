...
const WIRES: Wire[] = [
  {
    what: "جهوزيّةُ النشرة (`/ready`)",
    // ⚠️ `await readiness(` لا `readiness(`: النمطُ الثاني يطابق **تعريفَ
    // الدالّة نفسِه** في `modules/health/readiness.ts`، فيشهد الملفُّ
    // لنفسه وتمرّ البوّابةُ خضراءَ ولو لم يناديه مسارٌ قطّ. (قِيس: نُزع
    // نداءُ المسار فبقيت خضراء.) وصيغةُ النداء لا تقع في التعريف.
    needle: "await readiness(",
    breaks:
      "لا نقطةَ تقول إن كانت النشرةُ متجراً — فيُحكم على النشر بالظنّ، " +
      "وقد سقطت أوّلُ نشرةٍ حقيقيّة بخادمٍ **يستجيب** ومخطَّطٍ نصفَ مهاجَرٍ خلفَه.",
  },
  {
    what: "المطالبةُ بقطعةٍ من التخفيض الخاطف",
    needle: "await claimFlash(",
    breaks:
      "سقفُ الكمّية والنافذةُ وحدُّ العميل **حبرٌ**: العرضُ يُضبط في " +
      "اللوحة ولا مسارَ يطالب بقطعةٍ منه، فيُباع بعد النفاد وبعد " +
      "انتهاء الوقت — ولا يُستدرَك، إمّا شحنٌ بسعرٍ خاسرٍ أو إلغاءُ " +
      "طلبٍ قُبِل.",
  },
  {
    what: "ردُّ قطعةِ التخفيض الخاطف حين يسقط الإتمام",
    needle: "await releaseFlash(",
    breaks:
      "قطعةٌ طُولب بها قبل الطلب تبقى محبوسةً أبداً حين يسقط الإتمام — " +
      "فينفد عرضٌ **لم يُبَع**، ولا أحدَ يشتكي منه فلا يُكتشف.",
  },
  {
  ...
  {
    what: "تغييرُ كلمة المرور بإثبات الحاليّة",
    needle: "CURRENT_PASSWORD_WRONG",
    breaks:
      "لا مسارَ يسأل الكلمةَ الحالية — ومسارُ Medusa العامّ يأخذ الهويّةَ " +
      "من الرمز وحدَه، فمن وصل إلى جلسةٍ مسروقة يقفل الحسابَ على صاحبه.",
  },
  {
  ...
  {
    what: "كتابةُ سياسة الكوبون من مسارٍ إداريّ",
    needle: "createCouponPolicies(",
    breaks:
      "`zadim_coupon_policy` مبنيٌّ ومحروسٌ ومُختبَرٌ **ولا يضبطه أحد**: " +
      "الحدُّ لكل عميلٍ وسقفُ الخصم وأوّلُ طلبٍ حبرٌ حتى يُكتب صفٌّ بـ`psql`.",
  },
  {
    what: "قيدُ البيع في دفتر القيود",
    needle: "postSaleJournal(",
    breaks:
      "الدفترُ يبقى فارغاً بينما المتجرُ يبيع — **وبندان ينتظرانه**: " +
      "رصيدُ المتجر بدل النقد، وتقاريرُ الهامش. والأرقامُ تُشتقّ من " +
      "المجاميع، فلا سجلَّ يُحتكم إليه حين تختلف.",
  },
  {
    what: "إعادةُ تسليم ما لم يصل",
    needle: "redeliverPending(",
    breaks:
      "مزوّدٌ ساقطٌ عشرَ دقائق يعني رسائلَ تلك العشرِ **ضاعت إلى الأبد**: " +
      "صفٌّ حالتُه `failed` لا يقرؤه أحد، وحدثُ الصندوق وُسم مُسلَّماً فلا يُعاد.",
  },
  {
    what: "حارسُ إقفال الحساب على مسارات الدخول",
    needle: "guardLogin]",
    breaks:
      "حدُّ النداءات يحرس العنوانَ وحدَه: شبكةٌ بعشرة آلاف عنوانٍ تجرّب " +
      "حساباً واحداً تُنتج مئةَ ألفِ محاولةٍ في الدقيقة، **وكلُّها تحت الحدّ**. " +
      "والدفترُ يمتلئ ولا يقرؤه أحد.",
  },
  {
    what: "ترتيبُ المطالبة بالتخفيضات الخاطفة",
    // والتعريفُ عامٌّ (`orderByPriority<T extends …>`) فلا يُطابقه
    // «orderByPriority(» — الملفُّ لا يشهد لنفسه.
    needle: "orderByPriority(",
    breaks:
      "رقمُ الأولوية الذي يضبطه المديرُ **حبرٌ**: سلّةٌ فيها عرضان خاطفان " +
      "تُطالِب بأيّهما جاء أوّلاً في التسويات — ونفسُ السلّة بعد سقوطٍ " +
      "تُطالِب بغيره، فآخرُ قطعةٍ تذهب لعرضٍ لا يُتوقَّع.",
  },
  {
    what: "وسيطُ المراقبة على المسارات",
    // النداءُ هنا تركيبٌ في سلسلة الوسائط لا استدعاءٌ بقوسين —
    // و«observe(» وحدَه كان سيُطابق تعريفَه فيشهد الملفُّ لنفسه.
    needle: "[observe,",
    breaks:
      "لا سطرَ لأيّ طلب: لا معرّفٌ يُعاد للمشتكي، ولا مدّةٌ ولا حالةٌ " +
      "تُجمَع. وموجةُ ٤٠٣ أو ٤٢٩ تمرّ **بلا أثرٍ واحد**.",
  },
  {
